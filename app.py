...
<head>
    ...
    <style>
        ...
        .container { background: white; padding: 30px; border-radius: 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); display: inline-block; width: 80%; max-width: 700px; }
        h1 { color: #1F4E78; }
    ...
    <div class="container">
        ...
        {% if results %}
            <form action="/download" method="POST">
                <input type="hidden" name="data" value="{{ results | join('|||') }}">
                <button type="submit" class="excel-btn">📥 엑셀 파일로 받기</button>
            </form>
...
def home():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            results = [item.get_text() for item in items[1:16]] # 15개 수집
        except:
            results = ["데이터 수집 중 오류가 발생했습니다."]
    return render_template_string(HTML_TEMPLATE, results=results)

@app.route('/download', methods=['POST'])
def download():
    # 화면에 있는 데이터를 가져와서 엑셀로 만듭니다
    raw_data = request.form.get('data').split('|||')
    df = pd.DataFrame(raw_data, columns=['수집된 부동산 뉴스/시세 제목'])
    ...
    return send_file(output, as_attachment=True, download_name="부동산_데이터.xlsx", mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
